...
const app = {
   ...
   attachEventListeners() {
    ...
    async handleLogin(e) {
        ...
        try {
            ...
            if (data.status === 'success') {
                ...
                this.showError('login-error-msg', data.message || 'Invalid credentials');
            }
        } catch (e) {
            this.showError('login-error-msg', 'Server connection failed.');
        } finally {
            this.hideLoader();
        }
    },
    ...
    async handleSignup(e) {
        e.preventDefault();
        this.clearError('signup-error-msg');
        ...
        try {
            ...
            if (data.status === 'success') {
                ...
                this.showError('signup-error-msg', data.message);
            }
        } catch (e) {
            this.showError('signup-error-msg', 'Server connection failed.');
        } finally {
            this.hideLoader();
        }
    },
    ...
    // 9. Core Application Data Refreshing
    async refreshData() {
        const monthKey = this.getMonthKey();
        ...
        try {
            ...
            this.showToast("Failed to fetch recent data.", "error");
        } finally {
            this.hideLoader();
        }
        ...
        // Update DOM Stats using Currency Formatter
    ...
    async handleExpenseSubmit(e) {
        ...
        try {
            ...
            this.hideLoader(); // if refreshData runs, it manages its own loading state, so ok
        }
    },
    ...
    renderExpensesTable(expenses) {
        ...
        if (expenses.length === 0) {
            tbody.innerHTML = '<tr><td colspan="5" class="text-center text-muted" style="padding: 2rem; font-weight: 500;">No expenses found for this month</td></tr>';
            return;
        }
        ...
        sorted.forEach(exp => {
            ...
            tr.innerHTML = `
                ...
                <td class="actions">
                    <button class="btn-icon small text-primary hover-lift" title="Edit" onclick="app.editExpense('${exp.id}', \`${exp.title.replace(/`/g, "\\`")}\`, ${exp.amount}, '${exp.date}', '${exp.category}')">
                        <i class="ph ph-pencil-simple"></i>
                    </button>
                    <button class="btn-icon small text-danger hover-lift" title="Delete" onclick="app.deleteExpense('${exp.id}')">
                        ...
                        callbacks: {
                            label: function(context) {
                                ...
                                if (context.parsed.y !== null) {
                                    label += app.formatCurrency(context.parsed.y);
                                }
                                return label;
                            }
                        }
